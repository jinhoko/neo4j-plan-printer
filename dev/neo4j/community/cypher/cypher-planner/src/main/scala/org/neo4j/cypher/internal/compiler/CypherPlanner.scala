...
// JHKO
import org.neo4j.cypher.internal.compiler.planner.logical.debug.DebugPlanPrinter
...
case class CypherPlanner[Context <: PlannerContext](monitors: Monitors,
  ...
  def planPreparedQuery(state: BaseState, context: Context): LogicalPlanState = {
    val pipeLine = if(config.planSystemCommands)
      ...
      // JHKO add planprinter
      planPipeLine() andThen DebugPlanPrinter andThen DebugPrinter
    else
      // JHKO add planprinter
      planPipeLine() andThen DebugPlanPrinter
